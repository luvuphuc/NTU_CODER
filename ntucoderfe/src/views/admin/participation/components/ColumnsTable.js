...
export default function ParticipationTable({
  ...
  const [currentParticipationID, setCurrentParticipationID] = useState(null);

  const handleEditClick = (participationID) => {
  ...
  const handleDeleteClick = async () => {
    try {
      ...
      const response = await api.delete(
        `/Participation/${currentParticipationID}`,
      );
      if (response.status === 200) {
        toast({
          title: 'Xóa thành công!',
          description: 'Người dự thi đã bị xóa.',
          status: 'success',
          duration: 5000,
          isClosable: true,
          position: 'top-right',
      ...
      Cell: ({ row }) => (
        <Flex gap={4} justify="center" align="center">
          <Button
            ...
            borderRadius="md"
            minW="auto"
            onClick={() => handleEditClick(row.participationID)}
          >
            <BiEdit size="18" />
          </Button>
          <Button
            ...
            minW="auto"
            onClick={() => {
              setCurrentParticipationID(row.participationID);
              onOpen();
            }}
          >
            <MdDelete size="18" />
